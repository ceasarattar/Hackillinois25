...
namespace PkmngoBackendTesting
{
    ...
    {
        public PkmngoBackendTestingClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.PkmngoBackendTestingProgram.InitPlayer(accounts, levelSeed, programId);
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.PkmngoBackendTestingProgram.ChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.PkmngoBackendTestingProgram.SuperChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<PkmngoBackendTestingErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<PkmngoBackendTestingErrorKind>>{{6000U, new ProgramError<PkmngoBackendTestingErrorKind>(PkmngoBackendTestingErrorKind.NotEnoughEnergy, "Not enough energy")}, {6001U, new ProgramError<PkmngoBackendTestingErrorKind>(PkmngoBackendTestingErrorKind.WrongAuthority, "Wrong Authority")}, };
        }
    }
